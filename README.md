...
services:
  ...
  keycloak:
    ...
    entrypoint: /opt/keycloak/bin/kc.sh start-dev # Change `start-dev` to `start` for production
    volumes:
...
RUN curl -OL https://github.com/zorn-v/nextcloud-social-login/releases/download/v5.6.3/release.tar.gz
...
mv /usr/sociallogin /var/www/html/apps
chown -R www-data:www-data /var/www/html/apps/sociallogin
